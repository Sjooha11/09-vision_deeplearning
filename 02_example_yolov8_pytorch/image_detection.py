...
while cap.isOpened():
    ...
    for result in results: # 하나의 프레임
        ...
        for i in range(len(boxes)):  # 하나의 object
            ...
            conf_sc = str(round(boxes.conf[i].item(),2))
            ...
            cv2.putText(input_img, cls_name[cls_idx],(l_x,l_y-10),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2, cv2.LINE_AA)
            cv2.putText(input_img, conf_sc,(l_x+200,l_y-10),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,0,0),1, cv2.LINE_AA)
